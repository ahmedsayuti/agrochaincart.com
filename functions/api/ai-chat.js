...
export async function onRequestPost(context) {
  try {
    ...
    // 3. Call DeepSeek API – CORRECT ENDPOINT with /v1/
    ...
    if (!dsResponse.ok) {
      console.error('DeepSeek API error:', dsResponse.status, dsText);
      return jsonResponse({
        error: `DeepSeek error ${dsResponse.status}: ${dsText.slice(0, 200)}`
      }, 502);
    }
